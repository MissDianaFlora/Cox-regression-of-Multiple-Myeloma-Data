...
summary(allfit)$table

#survival plots
autoplot(allfit)
str(allfit)
ggsurvplot(allfit, conf.int = TRUE, surv.median.line = "hv", risk.table = "abs_pct",
           title = "Kaplan Meier Survival Estimate of MM patients Diagnosed between 2020-2022",
           xlab = "Time in Months",
...
#computing the univariate cox regression
ecog.cox <- coxph(Surv(survivalmonths, survival_status)~ Ecog_Index,  data =  initial_data)
bonepain.cox <- coxph(Surv(survivalmonths, survival_status)~ `Bone Pain`,  data =  initial_data)
creatinine.cox <- coxph(Surv(survivalmonths, survival_status)~ creatinine,  data =  initial_data)
treatment.cox <- coxph(Surv(survivalmonths, survival_status)~  Treatment,  data =  initial_data)
anemia.cox <- coxph(Surv(survivalmonths, survival_status)~  anemia,  data =  initial_data)
renalfailure.cox <- coxph(Surv(survivalmonths, survival_status)~  renalfailure,  data =  initial_data)
hypercalcemia.cox <- coxph(Surv(survivalmonths, survival_status)~ hypercalcemia,  data =  initial_data)

#survival variable
initial_data <- initial_data %>%
  mutate(survival = if_else(Survival %in% c("LTFU", "Alive"), "censored", "Deceased"))

summary(ecog.cox)
summary(bonepain.cox)
summary(creatinine.cox)
summary(treatment.cox)
summary(anemia.cox)
summary(renalfailure.cox)
summary(hypercalcemia.cox)

#extracting the cox regression output
...
# Extract the summary of the Cox regression model
summaryecog=summary(ecog.cox)
summarybone=summary(bonepain.cox)
summarycreatinine=summary(creatinine.cox)
summarytrt=summary(treatment.cox)
summaryanemia=summary(anemia.cox)
summaryrenal=summary(renalfailure.cox)
summaryhyp=summary(hypercalcemia.cox)

#computing Multivariate cox regression
mm.cox <- coxph(Surv(survivalmonths, survival_status)~ Ecog_Index + creatinine + Treatment + anemia +
                  renalfailure, data =  initial_data)
summary(mm.cox)
